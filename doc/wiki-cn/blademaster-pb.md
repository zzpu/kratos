...
* 编写`proto`文件，示例可参考[kratos-demo内proto文件](https://github.com/zzpu/kratos-demo/blob/master/api/api.proto)
...
建议在项目`api`目录下编写`proto`文件及生成对应的代码，可参考[kratos-demo内的api目录](https://github.com/zzpu/kratos-demo/tree/master/api)。
...
使用`RegisterDemoBMServer`示例代码请参考[kratos-demo内的http](https://github.com/zzpu/kratos-demo/blob/master/internal/server/http/server.go)内的如下代码：
...
`internal/service`内的`Service`结构实现了`DemoBMServer`接口可参考[kratos-demo内的service](https://github.com/zzpu/kratos-demo/blob/master/internal/service/service.go)内的如下代码：
